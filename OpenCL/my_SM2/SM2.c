...
/**
  ...
  */
EccPoint tmpPoint[1], tmpPoint1[1];
uint64_t tmpTimes[1];
...
/**
  ...
  */
uint8_t C[505];//这个函数地方malloc修改后健壮性太低了
...
/**
  ...
  */
  //这个修改malloc后健壮性也很低
uint8_t tmpRes[128],tmpData[132];
...
/**
  ...
  */
  //这个地方修改之后健壮性也低
uint8_t C2[409],x1[32],y1[32],x2[64],y2[32],t[408];
...
/**
  ...
  */
  //健壮性低
  extern SM2Signature result[1];
  uint8_t Z[210],ZA[32];
  uint8_t M_[46],e[32];
  uint8_t x1[32],y1[32],r[32],s[32];
  EccPoint point1[1];
...
/**
  ...
  */
   uint8_t Z1[212],ZA[32];
    uint8_t M_[46],e[32];
    EccPoint point0[1],point00[1],point1[1];
    uint8_t r[32];
BOOL SM2SignatureVerify(char* message, int messageSizeInBit, SM2Signature* sign, char* IDA, uint16_t ENTLAInBit, EccPoint* G, EccPoint* pubKey)
{
    ...
    //ZA = SM3(ENT LA ∥ IDA ∥ a ∥ b ∥ xG ∥ yG ∥ xA ∥ yA)。
    //printf("zm %d\n",2 + ENTLAInBit / 8 + 32 + 32 + 32 + 32 + 32 + 32);
    //uint8_t* Z = malloc(2 + ENTLAInBit / 8 + 32 + 32 + 32 + 32 + 32 + 32), * ZA = malloc(32);

    Z1[0] = ENTLAInBit >> 8;
    Z1[1] = ENTLAInBit & 0xff;
    memcpy(Z1 + 2, IDA, ENTLAInBit / 8);
    Uint256ToString(ZA, Ec->a);
    memcpy(Z1 + 2 + ENTLAInBit / 8, ZA, 32);
    Uint256ToString(ZA, Ec->b);
    memcpy(Z1 + 2 + ENTLAInBit / 8 + 32, ZA, 32);
    Uint256ToString(ZA, G->x);
    memcpy(Z1 + 2 + ENTLAInBit / 8 + 32 * 2, ZA, 32);
    Uint256ToString(ZA, G->y);
    memcpy(Z1 + 2 + ENTLAInBit / 8 + 32 * 3, ZA, 32);
    Uint256ToString(ZA, pubKey->x);
    memcpy(Z1 + 2 + ENTLAInBit / 8 + 32 * 4, ZA, 32);
    Uint256ToString(ZA, pubKey->y);
    memcpy(Z1 + 2 + ENTLAInBit / 8 + 32 * 5, ZA, 32);
    SM3(Z1, 2 + ENTLAInBit / 8 + 32 + 32 + 32 + 32 + 32 + 32, ZA);//ZA = SM3(ENT LA ∥ IDA ∥ a ∥ b ∥ xG ∥ yG ∥ xA ∥ yA)。
    //free(Z);
    //printf("M_%d\n",messageSizeInBit / 8 + 32);
    //uint8_t* M_ = malloc(messageSizeInBit / 8 + 32), * e = malloc(32);
...
BOOL KeyExchangeCheck = TRUE;//可选项，是否要验证S2 == SA以及S1 == SB
 EccPoint RA[1],V[1],U[1],pubKeyA[1],pubKeyB[1],RB[1];
  uint8_t tmp[212],tmp1[32];
  uint8_t SB[32],KB[16];
   uint8_t S1[32],KA[16];
   uint8_t SA[32],S2[32];
...
void SM2KeyExchange_AStep6To7(EccPoint* resultU, uint64_t* tA, EccPoint* RB, KeyExchangeAOriginalInfoDef infoA)
{
    ...
    CurvePointAdd(tmp12, resultU, infoA.PB);
    CurvePointMul(resultU, tmp12, tA);	//step7: U = [h*tA](PB+[x2_]*RB)这里的h我不会算
    //free(tmp);
}
...
{
    //uint8_t* tmpp = malloc(32 + 32 + 32 + 32), * tmp1 = malloc(32);	//tmp = xV||yV||ZA||ZB

    Uint256ToString(tmp1, U.x);
    memcpy(tmpp, tmp1, 32);
    Uint256ToString(tmp1, U.y);
    memcpy(tmpp + 32, tmp1, 32);
    memcpy(tmpp + 64, infoA.ZA, 32);
    memcpy(tmpp + 96, infoA.ZB, 32);
    KDF(KA, tmpp, (32 + 32 + 32 + 32) * 8, klenInBit);	//step7: 生成秘钥KB = KDF(xV||yV||ZA||ZB,klenInBit)
    if (KeyExchangeCheck)//需要校验S1==SB
    {
        //tmpp = realloc(tmpp, 32 * 7);
        Uint256ToString(tmp1, U.x);
        memcpy(tmpp, tmp1, 32);
        memcpy(tmpp + 32, infoA.ZA, 32);
        memcpy(tmpp + 64, infoA.ZB, 32);
        Uint256ToString(tmp1, RA.x);
        memcpy(tmpp + 96, tmp1, 32);
        Uint256ToString(tmp1, RA.y);
        memcpy(tmpp + 128, tmp1, 32);
        Uint256ToString(tmp1, RB.x);
        memcpy(tmpp + 160, tmp1, 32);
        Uint256ToString(tmp1, RB.y);
        memcpy(tmpp + 192, tmp1, 32);
        SM3(tmpp, 32 * 7, tmp1);	//tmp1 = Hash(xV ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2)

        tmpp[0] = 0x02;
        memcpy(tmpp + 1 + 32, tmp1, 32);
        Uint256ToString(tmp1, U.y);
        memcpy(tmpp + 1, tmp1, 32);
        SM3(tmpp, 65, S1);	//S1 =  Hash(0x02 ∥ yV ∥ Hash(xV ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2))
    }
    //free(tmpp);
    //free(tmp1);
}
...
/**
  ...
  */
  uint8_t tmppp[352],tmp1[32];
void SM2KeyExchange_AStep10(uint8_t* SA, EccPoint U, EccPoint RA, EccPoint RB, KeyExchangeAOriginalInfoDef infoA)
{
    //uint8_t* tmppp = malloc(32 + 32 + 32 + 32), * tmp1 = malloc(32);	//tmp = xV||yV||ZA||ZB

    if (KeyExchangeCheck)//需要校验S2==SA
    {
       // tmppp = realloc(tmppp, 32 * 7);
        Uint256ToString(tmp1, U.x);
        memcpy(tmppp, tmp1, 32);
        memcpy(tmppp + 32, infoA.ZA, 32);
        memcpy(tmppp + 64, infoA.ZB, 32);
        Uint256ToString(tmp1, RA.x);
        memcpy(tmppp + 96, tmp1, 32);
        Uint256ToString(tmp1, RA.y);
        memcpy(tmppp + 128, tmp1, 32);
        Uint256ToString(tmp1, RB.x);
        memcpy(tmppp + 160, tmp1, 32);
        Uint256ToString(tmp1, RB.y);
        memcpy(tmppp + 192, tmp1, 32);
        SM3(tmppp, 32 * 7, tmp1);	//tmp1 = Hash(xU ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2)

        tmppp[0] = 0x03;
        memcpy(tmppp + 1 + 32, tmp1, 32);
        Uint256ToString(tmp1, U.y);
        memcpy(tmppp + 1, tmp1, 32);
        SM3(tmppp, 65, SA);	//SA = Hash(0x02 ∥ yU ∥ Hash(xU ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2))
    }
    //free(tmppp);
    //free(tmp1);
}
...
void SM2KeyExchange_BStep7To9(uint8_t* KB, uint8_t* SB, EccPoint V, int klenInBit, EccPoint RA, EccPoint RB, KeyExchangeBOriginalInfoDef infoB)
{
    ...
    memcpy(tmpt + 32, tmp1, 32);
    memcpy(tmpt + 64, infoB.ZA, 32);
    memcpy(tmpt + 96, infoB.ZB, 32);
    KDF(KB, tmpt, (32 + 32 + 32 + 32) * 8, klenInBit);	//step7: 生成秘钥KB = KDF(xV||yV||ZA||ZB,klenInBit)
    if (KeyExchangeCheck)//需要校验S2==SA
    {
        //tmp = realloc(tmp, 32 * 7);
        Uint256ToString(tmp1, V.x);
        memcpy(tmpt, tmp1, 32);
        memcpy(tmpt + 32, infoB.ZA, 32);
        memcpy(tmpt + 64, infoB.ZB, 32);
        Uint256ToString(tmp1, RA.x);
        memcpy(tmpt + 96, tmp1, 32);
        Uint256ToString(tmp1, RA.y);
        memcpy(tmpt + 128, tmp1, 32);
        Uint256ToString(tmp1, RB.x);
        memcpy(tmpt + 160, tmp1, 32);
        Uint256ToString(tmp1, RB.y);
        memcpy(tmpt + 192, tmp1, 32);
        SM3(tmpt, 32 * 7, tmp1);	//tmp1 = Hash(xV ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2)

        tmpt[0] = 0x02;
        memcpy(tmpt + 1 + 32, tmp1, 32);
        Uint256ToString(tmp1, V.y);
        memcpy(tmpt + 1, tmp1, 32);
        SM3(tmpt, 65, SB);	// SB = Hash(0x02 ∥ yV ∥ Hash(xV ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2))
    }
    //free(tmp);
    //free(tmp1);
}
...
void SM2KeyExchange_BStep10(uint8_t* S2, EccPoint V, EccPoint RA, EccPoint RB, KeyExchangeBOriginalInfoDef infoB)
{
    ...
    if (KeyExchangeCheck)//需要校验S2==SA
    {
        //tmp = realloc(tmp, 32 * 7);
        Uint256ToString(tmp1, V.x);
        memcpy(tmptt, tmp1, 32);
        memcpy(tmptt + 32, infoB.ZA, 32);
        memcpy(tmptt + 64, infoB.ZB, 32);
        Uint256ToString(tmp1, RA.x);
        memcpy(tmptt + 96, tmp1, 32);
        Uint256ToString(tmp1, RA.y);
        memcpy(tmptt + 128, tmp1, 32);
        Uint256ToString(tmp1, RB.x);
        memcpy(tmptt+ 160, tmp1, 32);
        Uint256ToString(tmp1, RB.y);
        memcpy(tmptt + 192, tmp1, 32);
        SM3(tmptt, 32 * 7, tmp1);	//tmp1 = Hash(xV ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2)

        tmptt[0] = 0x03;
        memcpy(tmptt + 1 + 32, tmp1, 32);
        Uint256ToString(tmp1, V.y);
        memcpy(tmptt + 1, tmp1, 32);
        SM3(tmptt, 65, S2);	//tmp1 = SB = SB= Hash(0x02 ∥ yV ∥ Hash(xV ∥ ZA ∥ ZB ∥ x1 ∥ y1 ∥ x2 ∥ y2))
    }
    //free(tmp);
    //free(tmp1);
}
