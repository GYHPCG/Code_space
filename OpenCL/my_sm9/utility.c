...
extern BYTE* p4;
//对两个字节数组进行异或运算
void Bytes_XOR(BYTE *p4, BYTE *p1, BYTE *p3_,unsigned int len)
{
	int i;
	for(i=0;i<len;i++)
		p4[i] = p1[i]^p3_[i];
...
*/
BYTE msg[64];
void MAC(BYTE *p3_,BYTE *p4, unsigned len1,BYTE *p2, int len2)
{
	int len;
	//BYTE *msg;
	DWORD hash[8];
	len = len1 + len2;
	//printf("MAC's len is %d\n", len);
//	msg = (BYTE*)malloc(len);
	memcpy(msg,p4,len1);
	memcpy(&msg[len1],p2,len2);
	SM3_hash(msg, len, hash);
	SM3_Dword2byte(p3_,hash);
	//free(msg);
}
...
void PtoByte(BYTE *msg,BNPoint P)
{
   unsigned char *str;
...
*/
BYTE Msg2[455];
void KDF(BYTE *p1, BYTE *msg, unsigned int msgLength, unsigned int klen)
{
