...
void KA_Demo()
{
	...
	BYTE hash_byte[32],SB[32],hash_byte1[32];
	BYTE S1[32],SA[32],S2[32];
	...
	len1 = strlen((const char*)IDA);//5
	len1 +=1;
	//msg1 = (BYTE*)malloc(len1);//6
	BYTE msg1[6];
	...
	len2 = strlen((const char*)IDB);//3
	len2 +=1;
	//msg2 = (BYTE*)malloc(len2);//4
	BYTE msg2[4];
