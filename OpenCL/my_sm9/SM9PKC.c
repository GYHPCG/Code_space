...
void PKC_demo(int mode)
{
	...
	BYTE id[] = "Bob";//3
	BYTE M[] = "Chinese IBE standard";//20
	//BYTE *M_;
	//m = NULL;
	...
	time1 = (double)(finish-start);
	K2_len = 0x0100;
	idLen = strlen((const char*)id);//3
	mlen = strlen((const char*)M);//20
	//idLen = 3, mlen = 20;
    //公钥加密阶段
	...
	if(mode==0)          //序列方式加密
	{
		K2_len = K2_len/8;//0x0100 / 8            //计算字节数
	    klen = (mlen+K2_len)*8;//(20 + 0x0100 / 8)
        //K = (BYTE *)malloc(klen/8);
        //K[(20 + 0x0100 / 8) * 8];
        //K[416];
        p1 = K;
	    PKC_kem(p1,&C1,id,P1,P2,P_pub_e,klen);
//		P_toString(C1,HEX);
		//K1 = (BYTE *)malloc(mlen);
		//K1[20];
		memcpy(K1,K,mlen);
		...
		p1 = K1;
		p3_ = M;
		Bytes_XOR(p4,p1, p3_,mlen);
		//C3 = (BYTE *)malloc(32);
		//C3[32];

		p3_ = C3;
		MAC(p3_, p2,mlen,p2,K2_len);

	}else                    //分组方式加密
	{
		K1_len = 0x80;
		klen = K1_len + K2_len;//0x80 + 0x100
        //K = (BYTE *)malloc(48);
        //K[48];
       ...
       // K2 = (BYTE *)malloc(K2_len);
		...
		p4 = C2;
		p1 = K1;
	    sm4_setkey_enc(&ctx,p1);
	    sm4_crypt_ecb(&ctx,1,32,input,p4);
		//C3 = (BYTE *)malloc(32);
		//C3[32];
		p3_ = C3;
		MAC(p3_, p4,32,K2,K2_len);
	}
	finish = clock();
	time2 = (double)(finish-start);
	...
	}else                    //分组方式解密
	{
		...
		//printf("K_ klen %d\n", klen/8);
		//K_[klen / 8];
	    KDF(p1, msg, len, klen);  //注意此步骤还需要判断K是否为全零，否则重新生成
		...
		p1 = K1_;
		p4 = M_;
	    sm4_setkey_dec(&ctx,p1);
	    p1 = C2;
	    sm4_crypt_ecb(&ctx,0,32,p1,p4);
		//u = (BYTE *)malloc(32);
		//u[32];
		p2 = K2_;
		p3_ = u;
		p4 = C2;
		MAC(p3_, p4,32,p2,K2_len);
		//printf("m's len si %d\n", mlen);
		//m = (BYTE *)malloc(mlen);
	...
	}else
	{
		...
		printf("\n解密明文即为M_:");
        for (int i = 0; i< mlen; ++i)printf("%c", m[i]);
        printf("\n");
		//free(m);
	}
...
*/
BYTE msg1[451];
void PKC_kom(BYTE *p1, BNPoint C,BNPoint2 deB,BYTE *ID,unsigned int klen)
{
	BNField12 w;
	//BYTE *msg;
	...
	len2 = len1 + 64 + 384;
    //printf("PKC_kom's len2 is %d\n",len2);
	//msg = (BYTE*)malloc(len2);
	PtoByte(msg1,C);
	F12toByte(&msg1[64],w);
	memcpy(&msg1[448],ID,len1);
    KDF(p1, msg1, len2, klen);  //注意此步骤还需要判断K是否为全零，否则重新生成
	//free(msg);
}
