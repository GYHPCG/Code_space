...
int main()
{
    ...
    printf("pltfor's ret is %d\n", ret);
    ret = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_GPU, 1, &device_id, &ret_num_devices);
    printf("device si %d\n", ret);
    /* Create OpenCL context */
    context = clCreateContext(NULL, 1, &device_id, NULL, NULL, &ret);
    printf("context is ret %d\n", ret);
    /* Create Command Queue */
    command_queue = clCreateCommandQueue(context, device_id, 0, &ret);
    printf("commde's ret is %d\n", ret);
    ...
    printf("enque's ret is %d\n", ret);
    /* Create Kernel Program from the source */
    program = clCreateProgramWithSource(context, 1, (const char**)&source_str,
        (const size_t*)&source_size, &ret);
    printf("creat pro is %d\n", ret);
    /* Build Kernel Program */

   // ret = clBuildProgram(program, 1, &device_id, "-I ./ -cl-std=CL2.0", NULL, NULL);
    ret = clBuildProgram(program, 1, &device_id, NULL, NULL, NULL);
    printf("clBulid ret is %d\n", ret);
    if (ret != CL_SUCCESS) {
        char log[1024] = {0};
        clGetProgramBuildInfo(program, device_id, CL_PROGRAM_BUILD_LOG, 1024, log, NULL);
        //cout << "Build log:\n" << log << endl;
        printf("Bulid log:\n");
