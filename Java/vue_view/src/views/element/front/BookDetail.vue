<template>
    <div >
      ...
      overflow: hidden; text-overflow: ellipsis;">{{ book.introduction }}</h2>
         ...
         <div style="width: 300px; background-color: aqua; margin-top: 50px;">
       ...
       <div style="width: 700px; text-align: left; ">
            <el-button type="text" class="button_mod">想读</el-button>
...
export default {
    ...
    methods: {
      ...
      reply(pid, userName) {
        this.replyComment = {pid: pid, userId: 2, userName: '李四', bookId: 1, target: userName}
      },
      saveReply() {
        fetch("http://localhost:9099/api/comment",{
          method: 'post',
          headers: {
      ...
      load() {
        fetch("http://localhost:9099/api/comment?bookId=1").then(res => {
          if (res.status === 500) {
       ...
       submit() {
         ...
         this.comment.userName = "gg"
         this.comment.bookId = 1
        //  this.$http.post("/comment", JSON.stringify(this.comment)).then(res => {
