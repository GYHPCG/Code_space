 ...
 * @LastEditTime: 2023-12-01 20:50:03
-->
<template>
  <div style="width: 100%">
    <!-- 搜索框 -->
...
.test(value)) {
  ...
  methods: {
    ...
    load() {
      this.$http.get("/user/page", { params: this.params }).then((res) => {
    ...
    addUser(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http.post("/user/addUser", this.form).then((res) => {
    ...
    getInfo(id) {
      console.log(id);
      this.id = id;
      this.$http.get("/user/getInfo/" + id).then((res) => {
    ...
    updateInfo() {
      ...
      this.$http.put("/user/updateInfo", this.form).then((res) => {
        if (res.data.code === "200") {
    ...
    enable(row) {
      this.$http.put("/user/updateInfo", row).then((res) => {
        if (res.data.code === "200") {
    ...
    updatePasswd() {
      this.$refs['newFormRef'].validate((valid) => {
        if (valid) {
          ...
          this.$http.put('/user/updatePasswd', this.newForm).then(res => {
            if (res.data.code === '200') {
